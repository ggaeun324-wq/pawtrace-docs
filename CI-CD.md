...
|---|---|---|
| 배포 방식 | **롤링 업데이트 + 자동 롤백(서킷브레이커)** | Blue/Green(CodeDeploy) + 카나리 |
| 무중단 | min 100% / max 200% (새 태스크 먼저 기동) | 트래픽 가중 전환 |
| 마이그레이션 | 수동/설계 | CI에서 Alembic 자동 실행 |
| 환경 | 단일 | dev/staging/prod 분리 |
| 배포 후 검증 | 헬스체크 + 서킷브레이커 롤백 | 스모크 테스트 추가 |

> **자동 롤백**: ECS `deployment_circuit_breaker`가 켜져 있어, 새 배포가 안정화에 실패하면
> **직전 정상 버전으로 자동 복귀**합니다. 새 태스크를 먼저 띄운 뒤(최대 200%) 기존을 교체하므로
> `desired_count=1`에서도 무중단입니다.
